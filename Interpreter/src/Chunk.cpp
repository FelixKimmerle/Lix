...
unsigned int Chunk::disassembleInstruction(unsigned int p_uiOffset, unsigned int p_uiLine, unsigned int p_uiPrevline) const
{
	...
	case OpCode::OP_PUSH_FUNC:
		return constantInstruction("OP_PUSH_FUNC", p_uiOffset, LixDatatype::LD_FUNC);

	case OpCode::OP_CALL_FUNC:
		return callInstruction("OP_CALL_FUNC", p_uiOffset);
	...
	case OpCode::OP_POP_FUNC:
		return simpleInstruction("OP_POP_FUNC", p_uiOffset);
	case OpCode::OP_POP:
	...
	case OpCode::OP_GET_FUNC:
		return variableInstruction("OP_GET_FUNC", p_uiOffset);
...
unsigned int Chunk::callInstruction(const std::string &p_sName, unsigned int p_uiOffset) const
{
	u_int8_t offset = m_vCode[p_uiOffset + 1];
...
std::string Chunk::lixValueToString(LixDatatype p_Type, uint16_t p_uiOffset) const
{
	...
	case LixDatatype::LD_FUNC:
		ss << (*readConstant<Lix_Func>(p_uiOffset))->getName();
		break;
	}
	return ss.str();
}
...
template u_int16_t Chunk::addConstant<Lix_Func>(const Lix_Func p_Constant);
