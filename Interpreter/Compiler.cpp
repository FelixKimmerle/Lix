...
Compiler::Compiler(Logger *p_Logger) : m_bHadError(false), m_Logger(p_Logger)
{
}
...
void Compiler::visitFuncDecl(FuncDecl *p_pStmt)
{
    ...
    ObjFunction *func = comp.compile({p_pStmt->getBody()});
...
void Compiler::visitFunc(FuncExpr *p_pExpr)
{
...
ObjFunction *Compiler::compile(const std::vector<Stmt *> &p_vStmts)
{
    m_uiCurrentDepth = 0;
    for (auto &&stmt : p_vStmts)
    {
        stmt->visit(this);
    }
